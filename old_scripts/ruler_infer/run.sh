...
#
# RULER inference runner: ensure vLLM is up, then run prepare / call_api / evaluate.
# All config is defined below.
#
# Submit: cd A-RAVQA && sbatch scripts/ruler_infer/run.sh
# Run interactively: bash scripts/ruler_infer/run.sh
...
# Ensure logs dir exists for SLURM output (use job cwd; submit from repo root)
mkdir -p logs

# ---------- Paths ----------
ROOT_DIR="${REPO_ROOT}/outputs/0326/ruler"
# Model dir relative to RULER scripts (original used Qwen under third_party/RULER/scripts)
