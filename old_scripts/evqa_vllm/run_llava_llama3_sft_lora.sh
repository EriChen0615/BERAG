...
# Usage:
#   CHECKPOINT_PATH=/path/to/sft/checkpoint bash scripts/evqa_vllm/run_llava_llama3_sft_lora.sh

BASE_MODEL_PATH="${BASE_MODEL_PATH:-xtuner/llava-llama-3-8b-v1_1-transformers}"
PROCESSOR_PATH="${PROCESSOR_PATH:-${BASE_MODEL_PATH}}"
CHECKPOINT_PATH="${CHECKPOINT_PATH:-third_party/LLaMA-Factory-2502/saves/llava_llama3_8b_v1_1_transformers/lora/evqa_full/sft/rag5-answer-r64-b8-lr1e-5-max4096/checkpoint-7500}"
RETRIEVAL_DS_PATH="${RETRIEVAL_DS_PATH:-outputs/jinghong_chen/EVQA-testfull-with-retrieval_post_reranked}"
RETRIEVE_FIELD="${RETRIEVE_FIELD:-retrieved_passage}"
RETRIEVAL_TOPK_LIST=(1 2 3 5 10 15)
TAKE_N="${TAKE_N:-0}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/0526/EVQA/LLaVA-Llama3/VLLM/sft_lora}"
USE_CACHE="${USE_CACHE:-false}"
DO_EVAL="${DO_EVAL:-true}"
ENSURE_GT_PASSAGE_IN_ENSEMBLE="${ENSURE_GT_PASSAGE_IN_ENSEMBLE:-false}"

if [[ ! -d "${CHECKPOINT_PATH}" ]]; then
    echo "Missing SFT LoRA checkpoint directory: ${CHECKPOINT_PATH}" >&2
    exit 1
fi

if [[ ! -f "${CHECKPOINT_PATH}/adapter_config.json" ]]; then
    echo "Missing adapter_config.json in ${CHECKPOINT_PATH}" >&2
    exit 1
fi

if [[ ! -f "${CHECKPOINT_PATH}/adapter_model.safetensors" && ! -f "${CHECKPOINT_PATH}/adapter_model.bin" ]]; then
    echo "Missing LoRA adapter weights in ${CHECKPOINT_PATH}: expected adapter_model.safetensors or adapter_model.bin" >&2
    exit 1
fi
...
generate_exp_name() {
    ...
    local name="EVQA-VLLM-LLaVA-Llama3-SFT-LoRA-${checkpoint_name}-Top${topk}"
...
for retrieval_topk in "${RETRIEVAL_TOPK_LIST[@]}"; do
    ...
    echo "SFT LoRA checkpoint: ${CHECKPOINT_PATH}"
