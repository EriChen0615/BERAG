...
#SBATCH -p ampere
#SBATCH --array=0-8
#SBATCH --output=/home/jc2124/rds/rds-cvnlp-hirYTW1FQIw/shared_space/A-RAVQA/logs/slurm_infoseek_largek_%A_%a.out
#SBATCH --error=/home/jc2124/rds/rds-cvnlp-hirYTW1FQIw/shared_space/A-RAVQA/logs/slurm_infoseek_largek_%A_%a.err
...
# 7B BEFT checkpoint with prior head (l0h4) from existing Infoseek scripts.
...
EXP_NAME="InfoSeek-BAPE-LargeK-7B-l0h4-prior_head-K=${RETRIEVAL_TOPK}-TakeN=${TAKE_N}"
EXP_DIR="${OUTPUT_BASE}/${EXP_NAME}"
mkdir -p "${EXP_DIR}"

echo "[InfoSeek-LargeK] SLURM_ARRAY_TASK_ID=${TASK_ID}"
echo "[InfoSeek-LargeK] RETRIEVAL_TOPK=${RETRIEVAL_TOPK}"
echo "[InfoSeek-LargeK] EXP_DIR=${EXP_DIR}"
