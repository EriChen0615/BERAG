...
# Usage:
#   bash scripts/infoseek_bape/eval_llava_llama3_model_refactored_noprune.sh
#   CHECKPOINT_PATH=/path/to/checkpoint bash scripts/infoseek_bape/eval_llava_llama3_model_refactored_noprune.sh
#
# This wrapper is intended for evaluation environments. It reads cached
# inference_results.csv files produced by the matching run_llava... script.

BASE_MODEL_PATH="${BASE_MODEL_PATH:-xtuner/llava-llama-3-8b-v1_1-transformers}"
CHECKPOINT_PATH="${CHECKPOINT_PATH:-third_party/LLaMA-Factory-2502/saves/llava_llama3_8b_v1_1_transformers/lora/infoseek_new/beft/rag2-k2-prior=mlp-lr1e-6-h4-r64-size64000-max4096/checkpoint-6000}"
...
RETRIEVAL_TOPK_LIST=(1 5 10 15 20 30)
...
for retrieval_topk in "${RETRIEVAL_TOPK_LIST[@]}"; do
    ...
    result_csv="${OUTPUT_ROOT}/${full_exp_name}/inference_results.csv"

    if [[ ! -f "${result_csv}" ]]; then
        echo "Missing cached inference results for evaluation: ${result_csv}" >&2
        exit 1
    fi

    echo "--------------------------------"
    echo "Running evaluation for ${full_exp_name}"
    echo "Cached results: ${result_csv}"
    echo "Retrieval topk: ${retrieval_topk}"
    echo "Retrieval field: ${RETRIEVE_FIELD}"
    echo "Take N: ${TAKE_N}"
    echo "Offset: ${DS_OFFSET}"
    ...
    echo "Finished evaluation for ${full_exp_name}"
    echo "--------------------------------"
done
