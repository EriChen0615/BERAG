...
def initialize_bem_scoring_function(vocab_path=_VOCAB_PATH,
  ...
  def pad(a, length=512):
    # BEM expects shape (batch, 512) for both input_ids and segment_ids.
    # `a` is sometimes shaped like (1, seq_len); previous code sliced the first
    # axis and accidentally flattened, causing sequence lengths > 512 to slip
    # through (e.g., (1, 1043) -> (1043,)).
    arr = np.asarray(a)
    # Drop leading singleton batch dim if present, then ensure 1D for padding.
    arr = np.squeeze(arr)
    if arr.ndim != 1:
      raise ValueError(f"Array shape {arr.shape} is not 1D, flattening to 1D.")
      

    arr = arr.astype(np.int64, copy=False)
    truncated = arr[:length]
    pad_len = max(0, length - truncated.shape[0])
    if pad_len > 0:
      truncated = np.concatenate([truncated, np.zeros(pad_len, dtype=np.int64)], axis=0)
    return truncated
