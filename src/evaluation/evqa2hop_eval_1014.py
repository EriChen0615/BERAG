...
def process_row(row):
    ...
    if 'queries' in dir(row):
        ...
        for this_query, this_doc in zip(qs, docs):
            if any([this_doc.rstrip('\n') == gt_doc for gt_doc in gt_documents]):
                hit = True
            # if len(qs):
            #     print("qs:", qs)
...
def extract_queries_and_retrieved_docs(history):
    all_queries = []
    all_docs = []
...
if __name__ == '__main__':
    ...
    vqa_dataset  = load_vqa_dataset("EVQA2hop", split="test", img_basedir='data/')
    ...
    try:
        dict_to_report.update({
            "hit|call": sum(call_df["hit"])/len(call_df)
        })
    except:
        dict_to_report.update({
            "hit|call": "N/A"
