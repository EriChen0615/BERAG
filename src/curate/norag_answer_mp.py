...
def load_and_process_split(dataset, split_name, img_basedir, sample_size, rewrite_prompt_template):
    ...
    with ProcessPoolExecutor() as executor:
        results = list(
            tqdm(
                executor.map(process_dataset_item, dataset, chunksize=100000),
...
if __name__ == '__main__':
    ...
    train_data = load_and_process_split(vqa_dataset, 'train', args.img_basedir, args.sample_size_train, rewrite_prompt_template)
    if args.dataset_name == 'Infoseek':
        val_data = load_and_process_split(vqa_dataset, 'test', args.img_basedir, args.sample_size_eval, rewrite_prompt_template)
    else:
        val_data = load_and_process_split(vqa_dataset, 'valid', args.img_basedir, args.sample_size_eval, rewrite_prompt_template)
    test_data = load_and_process_split(vqa_dataset, 'test', args.img_basedir, args.sample_size_eval, rewrite_prompt_template)
