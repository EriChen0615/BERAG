...
class DatasetConverter:
    ...
    def _find_passage_medias(self, passages: Any | None) -> list[Any] | None:
        r"""Resolve passage-local image paths for BEFT ShareGPT examples."""
        if passages is None:
            return None

        if not isinstance(passages, list):
            raise ValueError("BEFT `passages` must be a list.")
        ...
        for passage in passages:
            ...
            resolved_passage = dict(passage)
            if "images" in resolved_passage:
                resolved_passage["images"] = self._find_medias(resolved_passage.get("images")) or []

            resolved_passages.append(resolved_passage)

        return resolved_passages
...
class SharegptDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        output = {
            ...
            "_videos": self._find_medias(example[self.dataset_attr.videos]) if self.dataset_attr.videos else None,
            "_audios": self._find_medias(example[self.dataset_attr.audios]) if self.dataset_attr.audios else None,
            "_passages": self._find_passage_medias(example.get(self.dataset_attr.passages))
            if self.dataset_attr.passages
            else None,
            "_gt_passage_idx": (
                example.get(self.dataset_attr.gt_passage_idx) if self.dataset_attr.gt_passage_idx else None
            ),
        }
        return output
