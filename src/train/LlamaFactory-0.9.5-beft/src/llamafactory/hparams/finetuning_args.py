...
class FinetuningArguments(
    ...
    stage: Literal["pt", "sft", "rm", "ppo", "dpo", "kto", "beft"] = field(
        default="sft",
        metadata={"help": "Which stage will be performed in training."},
    )
    beft_prior_modeling: Literal["none", "linear_head", "mlp_head"] = field(
        default="none",
        metadata={"help": "Prior head type for BEFT."},
    )
    beft_prior_head_num_layers: int = field(
        default=2,
        metadata={"help": "Number of layers in the BEFT prior head."},
    )
    beft_prior_head_proj_dim: int | None = field(
        default=None,
        metadata={"help": "Projection dimension in the BEFT MLP prior head. Defaults to model hidden size."},
    )
    beft_hidden_state_offset: int = field(
        default=0,
        metadata={"help": "Offset from the hidden state immediately before the first answer label for BEFT prior."},
    )
    beft_use_prior_head_loss: bool = field(
        default=False,
        metadata={"help": "Whether to supervise the BEFT prior head with multi-hot BCE labels."},
    )
    beft_prior_loss_factor: float = field(
        default=1.0,
        metadata={"help": "Loss weight for BEFT prior-head BCE supervision."},
    )
    beft_prior_head_lr: float | None = field(
        default=None,
        metadata={"help": "Learning rate for the BEFT prior head. Defaults to the main learning rate."},
    )
    beft_prior_head_path: str | None = field(
        default=None,
        metadata={"help": "Optional path to a saved BEFT prior_head.pt state dict."},
    )
    ...
    def __post_init__(self):
        ...
        if self.beft_prior_head_num_layers < 1:
            raise ValueError("`beft_prior_head_num_layers` must be greater than 0.")

        if self.beft_prior_head_proj_dim is not None and self.beft_prior_head_proj_dim < 1:
            raise ValueError("`beft_prior_head_proj_dim` must be greater than 0.")

        if self.beft_hidden_state_offset < 0:
            raise ValueError("`beft_hidden_state_offset` cannot be negative.")

        if self.beft_prior_head_lr is not None and self.beft_prior_head_lr <= 0:
            raise ValueError("`beft_prior_head_lr` must be greater than 0.")

        if self.beft_use_prior_head_loss and self.beft_prior_modeling == "none":
            raise ValueError("`beft_use_prior_head_loss` requires `beft_prior_modeling` to enable a prior head.")
